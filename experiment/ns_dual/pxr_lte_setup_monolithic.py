"""
Helper module for setting up custom namespace USD (pxr_lte) - MONOLITHIC BUILD.
...
This version is for the monolithic build (single lte_usd_ms.dll).

Usage:
    # Method 1: Import to auto-setup
    import pxr_lte_setup_monolithic
    from pxr import Usd as StandardUsd
    from pxr_lte import Usd as CustomUsd

    # Method 2: Explicit setup with custom paths
    import pxr_lte_setup_monolithic
    pxr_lte_setup_monolithic.setup(
        custom_usd_root="C:/path/to/dist-usd-lte-monolithic",
        tbb_root="C:/path/to/tbb"
    )
    from pxr_lte import Usd

Environment Variables:
    CUSTOM_USD_ROOT - Path to custom USD installation (default: ~/work/dist-usd-lte-monolithic)
...
# Default paths for MONOLITHIC build
DEFAULT_CUSTOM_USD_ROOT = Path.home() / "work" / "dist-usd-lte-monolithic"
...
def setup(custom_usd_root=None, tbb_root=None, verbose=False):
    """Set up paths for custom namespace USD (monolithic).
    ...
    Args:
        custom_usd_root: Path to custom USD installation.
                        Defaults to CUSTOM_USD_ROOT env var or ~/work/dist-usd-lte-monolithic
    ...
    if verbose:
        print(f"Setting up pxr_lte paths (MONOLITHIC):")
...
if __name__ == "__main__":
    ...
    print("pxr_lte_setup_monolithic - Dual USD Setup Helper (MONOLITHIC)")
    print("=" * 60)
    ...
    if pxr_lte_ok:
        print()
        print("SUCCESS: pxr_lte (monolithic) can be imported!")
        print()
        print("Example usage:")
        print("  import pxr_lte_setup_monolithic  # Auto-configures paths")
        ...
        print()
        print("Build USD with custom namespace (monolithic):")
