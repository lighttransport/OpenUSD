@echo off
REM Build C++ USD test with Visual Studio 2022 for pxr_lte MONOLITHIC
...
set "DIST_LTE=%USERPROFILE%\work\dist-usd-lte-monolithic"
...
echo ========================================
echo Building C++ USD Test - MONOLITHIC
echo ========================================
...
if not exist "%TBB_DIR%\include\tbb\tbb.h" (
    ...
    echo   build-tbb-prebuilt.bat
    exit /b 1
)

REM Verify monolithic library exists
if not exist "%DIST_LTE%\lib\lte_usd_ms.lib" (
    echo ERROR: Monolithic library not found at %DIST_LTE%\lib\lte_usd_ms.lib
    echo.
    echo This script requires the monolithic build.
    echo Use build_cpp_test.bat for the standard build.
    exit /b 1
)
...
echo.
echo ========================================
echo Building with Custom USD - MONOLITHIC
echo ========================================
...
cl /nologo %CXX_FLAGS% ^
    ...
    /I"%TBB_DIR%\include" ^
    /I"%UV_PYTHON_INCLUDE%" ^
    /Fo"%OUT_DIR%\test_usd_lte_monolithic.obj" ^
...
REM Link with monolithic library
echo.
echo [2/2] Linking with monolithic library...
link /nologo ^
    /OUT:"%OUT_DIR%\test_usd_lte_monolithic.exe" ^
    /LIBPATH:"%DIST_LTE%\lib" ^
    /LIBPATH:"%TBB_DIR%\lib" ^
    /LIBPATH:"%UV_PYTHON_LIBS%" ^
    "%OUT_DIR%\test_usd_lte_monolithic.obj" ^
    lte_usd_ms.lib ^
...
if errorlevel 1 (
    echo Link failed!
    echo.
    echo Note: Make sure the monolithic build is complete.
...
echo Build Complete
echo ========================================
echo Output: %OUT_DIR%\test_usd_lte_monolithic.exe
echo.
...
"%OUT_DIR%\test_usd_lte_monolithic.exe"
