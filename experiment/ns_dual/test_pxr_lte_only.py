#!/usr/bin/env python3
"""
Test script for custom namespace USD only (pxr_lte v26.05).

This script tests only the custom namespace USD build without requiring
standard pxr to be installed. Use this to verify your custom build works.

Requirements:
- Custom namespace USD v26.05: built from build-usd-lte.bat

Usage:
    python test_pxr_lte_only.py

The script will:
1. Set up DLL and Python paths for pxr_lte
...
def setup_paths():
    """Set up DLL and Python paths for pxr_lte."""

    print("Setting up paths for pxr_lte...")
...
def test_stage_creation():
    ...
    stage.SetMetadata("comment", "Test stage from pxr_lte v26.05")
...
def test_export():
    ...
    stage = Usd.Stage.CreateInMemory()
    stage.SetMetadata("comment", "Generated by pxr_lte v26.05 test")
...
def main():
    ...
    print("=" * 60)
    print("pxr_lte Only Test (USD v26.05 Custom Namespace)")
    print("=" * 60)
    print()
    ...
    print(f"pxr_lte v26.05 is working correctly.")
    print(f"  Version: {Usd.GetVersion()}")
    print("=" * 60)
