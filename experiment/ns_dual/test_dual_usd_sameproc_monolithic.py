#!/usr/bin/env python3
"""
Test script for same-process dual USD usage (v24.11 MONOLITHIC).

This script demonstrates importing both standard USD (pxr) and custom
namespace USD (pxr_lte v24.11 monolithic) in the same Python process.

Requirements:
- Standard USD: pip install usd-core
- Custom namespace USD v24.11 (monolithic): built from build-usd-lte-monolithic.bat
...
# Custom namespace USD v24.11 MONOLITHIC build paths
...
def test_imports():
    ...
    # Import custom namespace USD v24.11 (monolithic)
    print("\n2. Importing pxr_lte (custom namespace USD v24.11 MONOLITHIC)...")
    try:
...
def test_stage_creation(StandardUsd, CustomUsd):
    ...
    print("\n2. Creating custom namespace USD v24.11 stage (monolithic)...")
    try:
...
def test_independence(std_stage, custom_stage, StandardUsd, CustomUsd):
    ...
    print("\n2. Custom v24.11 (monolithic) stage prims:")
...
def test_export(std_stage, custom_stage):
    ...
    print("2. Custom USD v24.11 (monolithic) USDA output:")
    print("-" * 40)
...
def main():
    ...
    print("=" * 60)
    print("Same-Process Dual USD Test (v24.11 MONOLITHIC)")
    print("=" * 60)
    print()
    ...
    if CustomUsd is None:
        ...
        print("\nMake sure to build USD v24.11 with custom namespace (monolithic):")
    ...
    print(f"  pxr_lte (custom v24.11 monolithic): {CustomUsd.GetVersion()}")
    print("=" * 60)
