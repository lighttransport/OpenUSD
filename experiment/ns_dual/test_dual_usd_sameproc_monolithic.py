#!/usr/bin/env python3
"""
Test script for same-process dual USD usage - MONOLITHIC BUILD.

This script demonstrates importing both standard USD (pxr) and custom
namespace USD (pxr_lte monolithic) in the same Python process.

Requirements:
- Standard USD: pip install usd-core
- Custom namespace USD (monolithic): built from build-lte-monolithic.bat

Usage:
    python test_dual_usd_sameproc_monolithic.py
...
# Custom namespace USD MONOLITHIC build paths
CUSTOM_USD_ROOT = os.path.expandvars(r"$USERPROFILE\work\dist-usd-lte-monolithic")
...
def setup_paths():
    ...
    print("Setting up paths (monolithic)...")
...
def test_imports():
    ...
    # Import custom namespace USD (monolithic)
    print("\n2. Importing pxr_lte (custom namespace USD MONOLITHIC)...")
    try:
...
def test_stage_creation(StandardUsd, CustomUsd):
    ...
    # Create custom namespace USD stage (monolithic)
    print("\n2. Creating custom namespace USD stage (monolithic)...")
    try:
...
def test_independence(std_stage, custom_stage, StandardUsd, CustomUsd):
    ...
    print("\n2. Custom (monolithic) stage prims:")
...
def test_export(std_stage, custom_stage):
    ...
    print("2. Custom USD (monolithic) USDA output:")
    print("-" * 40)
...
def main():
    ...
    print("=" * 60)
    print("Same-Process Dual USD Test (MONOLITHIC)")
    print("=" * 60)
    print()
    ...
    if CustomUsd is None:
        ...
        print("\nMake sure to build USD with custom namespace monolithic:")
        print("  configure-lte-monolithic.bat")
        print("  build-lte-monolithic.bat all")
        return 1
    ...
    print("Same-process dual USD (monolithic) is working correctly.")
    if StandardUsd:
        print(f"  pxr (standard): {StandardUsd.GetVersion()}")
    print(f"  pxr_lte (monolithic): {CustomUsd.GetVersion()}")
    print("=" * 60)
